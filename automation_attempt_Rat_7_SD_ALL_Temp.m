...
root_rat= '/media/irene/GL14_RAT_FANO/Spikesorting/Rat_OS_Ephys_RGS14_rat7_373727';
...
Temp_Study_Days=study_days;%([1 3 4 5]); % TEMPORARY
...
for study_days_index=1 :length(Temp_Study_Days) 
    disp(study_days_index)    
    root_prelim=strcat(root_rat,{'/'},string(Temp_Study_Days(study_days_index)));
    ...
    %% Getting folders for tetrodes (accomodates for hpc)
      ...
      k1=strfind(root_rat,'_rat');%rat number
      k2=strfind(R,'_SD');
      ...
      name_exp_cond_prelim=string(string(R(k2(1)+5:k2(1)+8)))%gives a string containing the experimental conditons (For two character conditions, extra '_')
      name_exp_cond_final=regexp(name_exp_cond_prelim,'_','split');
      if study_days_index<length(Temp_Study_Days)
          name_exp_cond_final=name_exp_cond_final{1}
      
      else
          name_exp_cond_final=name_exp_cond_final{2}
      end
      disp(strcat('starting extraction for:','SD',string(R(k2(1)+3))))
      ...
      for folder=1:length(tetrodes)
          ...
          for channel_index=1:length(Channels_array)
              ...
              for curUnitInd=1:numUnits %from 1 to 8
                  ...
                  W_temp(curUnitInd,channel_index).WFM_Titles=strcat('RGS_PROJECT_RGS14_PFC','_','Rn',root_rat(k1+4),'_',string(name_exp_cond_final),'_','SD',string(study_day),'_','T',string(T{1}{2}),'_','UID',string(curUnitID),'_','C',string(channel_index));%%k2(1),k1, T(2) may give array indexing errors; best to check their contents
...
RAT_7_TEMP_SD_BEST_WFM= Stored_WFMs;
% for k=1:size(Stored_WFMs,1)
% Stored_WFMs(k).WFM_Titles=strrep(Stored_WFMs(k).WFM_Titles,'PROJECT_RGS','PROJECT_RGS14');
% end
RAT_7_TEMP_SD_BEST_WFM= Stored_WFMs;
save('/home/irene/Downloads/RGS14_all_Shekhar/Code/RAT_7_TEMP_SD_BEST_WFM');         
% et=toc/60;
