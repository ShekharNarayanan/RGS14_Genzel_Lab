...
root_rat= '/media/irene/MD04_RAT_SPIKE/Spike_sorting/Rat_OS_Ephys_RGS14_rat3_357152';
...
Temp_Study_Days=study_days([7 1 3 4 5 6]); % TEMPORARY
...
for study_days_index=1:length(Temp_Study_Days) 
    ...
    addpath(root_merged)
    load('trials_durations_samples.mat'); %used for calculating average firing rate
    ...
    %% Getting folders for tetrodes (accomodates for hpc)
      ...
      name_exp_cond_prelim=string(string(R(k2(1)+4:k2(1)+7)));%gives a string containing the experimental conditons (For two character conditions, extra '_')
...
for folder=1:length(tetrodes)
    ...
    W_temp=[];
        ...
        for channel_index=1:length(Channels_array)
            A=strcat(root,{'/'},string(tetrodes(folder)));
            cd(A)
...
RAT_3_TEMP_SD_BEST_WFM= Stored_WFMs;
save('/home/irene/Downloads/RGS14_all_Shekhar/Code/RAT_3_TEMP_SD_BEST_WFM');         
et=toc/60;
disp(strcat('Run time in minutes:',string(et)))
%sendmail('relativelygenius1111@gmail.com',strcat('Rat',root_rat(k1+4)))
