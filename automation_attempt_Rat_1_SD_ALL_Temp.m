...
root_rat= '/media/irene/GL04_RAT_HOMER_2/Spike_sorting/Rat_OS_Ephys_RGS14_Rat1_57986';
...
for study_days_index=1:length(Temp_Study_Days) 
    ...
    %% Getting folders for tetrodes (accomodates for hpc)
      ...
      name_exp_cond_prelim=R(k2(1)+5:k2(1)+7);%gives a string containing the experimental conditons (For two character conditions, extra '_')
      ...
      for folder=1:length(tetrodes)
          ...
          for channel_index=1:length(Channels_array)
              ...
              for curUnitInd=1:numUnits %from 1 to 8
                  ...
                  W_temp(curUnitInd,channel_index).WFM_Titles=strcat('RGS_PROJECT_VEHICLE_PFC','_','Rn',R(k1(1)+4),'_',string(name_exp_cond_final),'_','SD',string(study_day),'_','T',string(T{1}{2}),'_','UID',string(curUnitID),'_','C',string(channel_index));%%k2(1),k1, T(2) may give array indexing errors; best to check their contents
...
Stored_WFMs(k).WFM_Titles=strrep(Stored_WFMs(k).WFM_Titles,'Rn','Rn1');
end
RAT_1_TEMP_SD_BEST_WFM= Stored_WFMs;
% save('/home/irene/Downloads/RGS14_all_Shekhar/Code/RAT_1_TEMP_SD_BEST_WFM');         
% et=toc/60;
