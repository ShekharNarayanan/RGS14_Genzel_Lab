...
%             waveFormsMean = nan(nChInMap,wfNSamples,numUnits);%[1 82 8]
            ...
            for curUnitInd=1:numUnits %from 1 to 8
                ...
                W_temp(curUnitInd,channel_index).WFM_Titles=strcat('RGS_PROJECT_VEHICLE_PFC','_','Rn',root_rat(k1+4),'_',string(name_exp_cond_final),'_','SD',string(study_day),'_','T',string(T{1}{2}),'_','UID',string(curUnitID),'_','C',string(channel_index));%%k2(1),k1, T(2) may give array indexing errors; best to check their contents 
...
% load('RAT_2_TEMP_SD_BEST_WFM.mat')
% for k=1:size(Stored_WFMs,1)
% Stored_WFMs(k).WFM_Titles=strrep(Stored_WFMs(k).WFM_Titles,'WFM','RGS_PROJECT_VEHICLE_PFC');
% end
RAT_2_TEMP_SD_BEST_WFM= Stored_WFMs;
% save('/home/irene/Downloads/RGS14_all_Shekhar/Code/RAT_2_TEMP_SD_BEST_WFM');         
% et=toc/60;
% disp(strcat('Run time in minutes:',string(et)))
%sendmail('relativelygenius1111@gmail.com','Script Status','RAT_2 OVER')
