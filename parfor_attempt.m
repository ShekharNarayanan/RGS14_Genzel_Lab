...
root_merged=[];
for study_days_index=2:length(study_days) 
    %if study_days_index==2    
...
root_cortex=strcat(root_merged,{'/'},'cortex');

root=root_cortex;

parfor root_index=1:length(root)
    cd(root(root_index))
    
    %if root_index==1
tetrodes_dir=dir(root(root_index));
...
R=convertStringsToChars(root(root_index));
...
for folder=1:length(tetrodes)
    ...
    W_temp=[];
        ...
        cd (strcat(R,{'/'},string(tetrodes(folder))))
        ...
        for channel_index=1:length(Channels_array)
            ...
            params_dir= dir(strcat(R,{'/'},string(tetrodes(folder)))); %Reaching the directory after user input
            % remove all files (isdir property is 0)
            ...
            gwfparams=struct('dataDir',strcat(R,{'/'},string(tetrodes(folder)),{'/'},extension_final));  %entering extension_final
...
%             waveFormsMean = nan(nChInMap,wfNSamples,numUnits);%[1 82 8]
            ...
            for curUnitInd=1:numUnits %from 1 to 8
                ...
                W_temp(curUnitInd,channel_index).WFM_Titles=strcat('WFM','_','Rn',rat_name,'_',string(name_exp_cond_final),'_','SD',string(study_day),'_','T',string(T{1}{2}),'_','UID',string(curUnitID),'_','C',string(channel_index));%%k2(1),k1, T(2) may give array indexing errors; best to check their contents 
