...
%Folder with phy data (Change to your path)
%gwfparams.dataDir='/home/irene/Downloads/Tetrodes/Tetrode_8/phy_AGR';
...
parfor curUnitInd=linspace(1,numUnits,numUnits) %from 1 to 8
...
%WFM_3=waveFormsMean_3;
 ...
 for Unit=1:numUnits 
        subplot(2,2,Unit)
        plot(((0:length(waveFormsMean_4(:,:,Unit))-1))/30000*1000,waveFormsMean_4(:,:,Unit))
        xlabel('millisecond');
        ylabel('Unit ID'+string(unitIDs(Unit)));
 end     
 
C=[];
for i=1:size(waveFormsMean_4,3)
    B.title=strcat('WFM_4 ID',string(i));
    B.data=waveFormsMean_4(:,:,i);
    C=[C;B];
end    
...
%WFM_ALL_DC=[WFM_1;WFM_2;WFM_3;WFM_4];%concatenating all mean waveforms
%save('/home/irene/Downloads/Tetrode_8/Code/WFM_ALL');


 %plot(((0:length(X(1,:).data)-1)*1000/30000),X(1,:).data)
