...
root_rat= '/media/irene/GL13_RAT_BURSTY/Spike_sorting/Rat_OS_Ephys_RGS14_rat4_357153';
...
%             waveFormsMean = nan(nChInMap,wfNSamples,numUnits);%[1 82 8]
            ...
            for curUnitInd=1:numUnits %from 1 to 8
                ...
                W_temp(curUnitInd,channel_index).WFM_Titles=strcat('WFM','_','Rn',root_rat(k1+4),'_',string(name_exp_cond_final),'_','SD',string(study_day),'_','T',string(T{1}{2}),'_','UID',string(curUnitID),'_','C',string(channel_index));%%k2(1),k1, T(2) may give array indexing errors; best to check their contents 
...
%RAT_4_TEMP_SD_BEST_WFM= Stored_WFMs;
for k=1:size(Stored_WFMs,1)
Stored_WFMs(k).WFM_Titles=strrep(Stored_WFMs(k).WFM_Titles,'PROJECT_RGS','PROJECT_RGS14');
end
RAT_4_TEMP_SD_BEST_WFM= Stored_WFMs;
save('/home/irene/Downloads/RGS14_all_Shekhar/Code/RAT_4_TEMP_SD_BEST_WFM'); 
       
et=toc/60;
disp(strcat('Run time in minutes:',string(et)))
%sendmail('relativelygenius1111@gmail.com','Script Status',string(R(k1+4)))