...
numUnits = size(unitIDs,1);% size 8
spikeTimeKeeps = nan(numUnits,gwfparams.nWf);%[8 2000]
waveForms_2 = nan(numUnits,gwfparams.nWf,nChInMap,wfNSamples);%**[8 2000 1 82](single channel)
waveFormsMean_2 = nan(numUnits,nChInMap,wfNSamples); %[8 x82 x1]
...
for curUnitInd=1:numUnits %from 1 to 8
    ...
    for curSpikeTime = 1:min([gwfparams.nWf curUnitnSpikes])
        ...
        waveForms_2(curUnitInd,curSpikeTime,:,:) = tmpWf; %seems to work; [1 1 1 82]=[82] **
        
    end
    
    waveFormsMean_2(curUnitInd,:,:) = squeeze(nanmean(waveForms_2(curUnitInd,:,:,:),2));
    %waveFormsMean_2(curUnitInd,:,:) = squeeze(nanmean(waveForms_2(curUnitInd,:,:,:),2));% compute without mean
    disp(['Completed ' int2str(curUnitInd) ' units of ' int2str(numUnits) '.']);
end 


%WFM_1=waveFormsMean_1;
WFM_2=waveFormsMean_2;
%WFM_3=waveFormsMean_3;
%WFM_4=waveFormsMean_4;


%WFM_ALL=[WFM_1,WFM_2];
    for Unit=1:numUnits 
        subplot(4,4,Unit)
        plot(((0:length(WFM_2(:,:,Unit))-1))/30000*1000,WFM_2(:,:,Unit))
        xlabel('millisecond');
        ylabel('Unit ID'+string(unitIDs(Unit)));
    end    

%Res=reshape(WFM_1,[656,1]);%loop this for reshaping
%A = {'Channel_1';Res};
%filename = '/home/irene/Downloads/Tetrode_8/Code/testdata.xlsx';
%writecell(A,filename,'Sheet',1,'Range','A1:A700');
%open('testdata.xlsx')